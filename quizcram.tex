...
To compare how users interacted with the two tools, we logged the users' interactions as they studied the lectures, as shown in \autoref{fig:event-logs}. We found that users answered each question more times when using QuizCram. Users also reviewed previously-answered questions more often when using QuizCram. This increase in practice and reviewing helps explain the increased exam scores on the original in-video questions. %during the exams. % when using QuizCram.
