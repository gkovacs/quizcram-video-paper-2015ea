...
QuizCram's interface shows users a question to review, with an associated video segment. The system includes features a recommendation system for suggesting questions to review, interface to help users track their progress in videos, 
...
Of course, immediately showing the answer in response to an incorrect answer leads to the risk that learners may choose to immediately reveal the answer without attempting to answer the question themselves. To discourage such behavior, even though we show the user the answer and explanation in response to an incorrect response, we do not advance them automatically. Rather, we shuffle the answer options and require them to view at least 10\% of the video segment, which is roughly 20 seconds, before attempting to answer it again, as shown in Figure~\ref{fig:figure2}. This viewing task encourages users to review the video, incentivizes users to answer questions correctly, and ensures they aren't simply storing the answers in short-term memory and reproducing them. Requiring users to review the video and then retesting them after an incorrect response creates an additional retrieval opportunity, which should improve retention of the material \cite{testingeffect}.
...
\caption{The timeline displays the past set of questions the user has answered. We list the correctness score and video progress scores to help users locate the questions they had difficulty with, and videos they have not yet fully watched.}
\label{fig:figure3}
\end{figure*}

Although QuizCram focuses the user's attention towards the current question and associated video segment, we also wish to make it easy to refer to the previously answered video segments. This gives users finer granularity than simply listing the videos as Coursera does. As shown in Figure~\ref{fig:figure3}, the timeline displays the past set of questions that the user has answered, along with a minified version associated video, immediately below, so they can be conveniently accessed by scrolling down. Clicking on the video enlarges it to full-size and allows the user to rewatch it.

\subsection{Visualizing progress through videos}

With QuizCram, we hope to make reviewing more convenient than 

\subsection{Visualizing progress through the material}
